 ...
 */
class FileListViewController: BaseTableViewController {

    // MARK: - Constants

    /// 画面タイトル
    private let kScreenTitle = "一覧"
    ...
    override func rightBarButtonPressed(buttonItem: UIBarButtonItem) {
        // ファイル作成画面に遷移する。
        let vc = FileCreateViewController()
 ...
 　  */
    func swipeableTableViewCell(cell: SWTableViewCell, didTriggerRightUtilityButtonWithIndex index: NSInteger) {
        ...
        case 0:
            ...
            let vc = FileRenameViewController(oldFileName: oldFileName)
            navigationController?.pushViewController(vc, animated: true)
            break
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        // 編集画面に遷移する。
        let fileName = fileInfo.name
        let vc = FileEditViewController(fileName: fileName)
        navigationController?.pushViewController(vc, animated: true)
    }
